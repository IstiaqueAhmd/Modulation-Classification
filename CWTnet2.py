...
class TripleStreamCWTNet(nn.Module):
    def __init__(self, num_classes, dropout=0.4):
        ...
        self.stream_freq = make_stream()  # new third stream

        # Cross-attention modules
        self.cross_amp_phase = CrossAttention(64)
        self.cross_phase_freq = CrossAttention(64)
        self.cross_amp_freq = CrossAttention(64)
    ...
    def forward(self, x):
        """
        Expects input shape: [B, 3, H, W]
        where channels are [Amplitude, Phase, Frequency]
        """
        xa = self.stream_amp(x[:, 0:1])   # amplitude
        xp = self.stream_phase(x[:, 1:2]) # phase
        xf = self.stream_freq(x[:, 2:3])  # frequency

        # Pairwise cross-attention
        a_p = self.cross_amp_phase(xa, xp)
        p_f = self.cross_phase_freq(xp, xf)
        a_f = self.cross_amp_freq(xa, xf)

        # Combine fused features
        fused = (a_p + p_f + a_f) / 3.0
...
if __name__ == '__main__':
    ...
    data_dir = f'Scalograms_v2/snr_{SNR}'
    ...
    print(device)
    model = TripleStreamCWTNet(len(train_dataset.classes)).to(device)
