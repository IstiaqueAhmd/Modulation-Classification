...
"""Modification 1: Frequency Time Aware Convolutions"""
...
class FreqTimeBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.freq_conv = nn.Sequential(
            nn.Conv2d(in_ch, in_ch, kernel_size=(5, 1), padding=(2, 0), groups=in_ch, bias=False),
            nn.BatchNorm2d(in_ch),
            nn.ReLU(inplace=True)
        )
        self.time_conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=(1, 7), stride=(1, 2), padding=(0, 3), bias=False),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class SingleStreamCNN(nn.Module):
    def __init__(self, num_classes, dropout=0.3):
        ...
        # We modify the input of the first block to 2 channels
        # to take [Amplitude, Phase] simultaneously.
        self.features = nn.Sequential(
            FreqTimeBlock(2, 16), # Modified in_ch from 1 to 2
            nn.MaxPool2d(3, 2),
            DSConv(16, 32),
            nn.MaxPool2d(3, 2),
            DSConv(32, 64),
...
if __name__ == "__main__":
    ...
    MODEL_PATH = "best_model_multisnr_singlestream.pth"
    STATS_PATH = "normalization_stats_multisnr_singlestream.json"
    ...
    if TRAIN:
        ...
        model = SingleStreamCNN(num_classes=num_classes).to(DEVICE)
        ...
        print(f"Model has {num_params:,} parameters (SingleStreamCNN).")
    ...
    plt.ylabel('True')
    plt.tight_layout()
    plt.savefig(f'singlestream_confusion_matrix_test_snr{TEST_SNR}.png')
    plt.close()
    print(f"Saved singlestream_confusion_matrix_test_snr{TEST_SNR}.png")
