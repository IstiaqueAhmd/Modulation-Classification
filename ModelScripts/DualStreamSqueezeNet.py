...
# ==========================================
# 1. Lightweight Dual Stream SqueezeNet Architecture
# ==========================================

class DualStreamRadioSqueezeNet(nn.Module):
    def __init__(self, num_classes):
        super(DualStreamRadioSqueezeNet, self).__init__()
        
        # ============================================
        # Stream 1: Amplitude Branch (SqueezeNet 1.1)
        # ============================================
        # We load the feature extractor only (layers before the classifier)
        base_amp = models.squeezenet1_1(weights=None)
        
        # Modify the first layer to accept 1 channel instead of 3
        # Original: nn.Conv2d(3, 64, kernel_size=3, stride=2)
        # New:      nn.Conv2d(1, 64, kernel_size=3, stride=2)
        base_amp.features[0] = nn.Conv2d(1, 64, kernel_size=3, stride=2)
        
        self.stream_amp = base_amp.features
        
        # ============================================
        # Stream 2: Phase Branch (SqueezeNet 1.1)
        # ============================================
        base_phase = models.squeezenet1_1(weights=None)
        
        # Modify the first layer for 1 channel
        base_phase.features[0] = nn.Conv2d(1, 64, kernel_size=3, stride=2)
        
        self.stream_phase = base_phase.features
        ...
        # 512 (Amp) + 512 (Phase) = 1024 features
        self.fusion_dim = 512 + 512
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Dropout(0.5), # SqueezeNet usually benefits from 0.5 dropout
            nn.Linear(self.fusion_dim, num_classes)
        )

    def forward(self, x):
        # x shape: [Batch, 2, 224, 224]
        
        # 1. Split streams
        x_amp = x[:, 0:1, :, :]   # Channel 0
        x_phase = x[:, 1:2, :, :] # Channel 1
        
        # 2. Extract Features (Output: [Batch, 512, 13, 13] for 224x224 input)
        feat_amp = self.stream_amp(x_amp)
        feat_phase = self.stream_phase(x_phase)
        
        # 3. Global Average Pooling -> [Batch, 512, 1, 1]
        feat_amp = self.pool(feat_amp)
        feat_phase = self.pool(feat_phase)
        
        # 4. Flatten -> [Batch, 512]
        feat_amp = torch.flatten(feat_amp, 1)
        feat_phase = torch.flatten(feat_phase, 1)
        
        # 5. Concatenate (Fuse) -> [Batch, 1024]
        combined_features = torch.cat((feat_amp, feat_phase), dim=1)
...
if __name__ == '__main__':
    ...
    data_dir = f'{DATA_DIR}/snr_{SNR}' 
    
    print(f"--- Processing Data from: {data_dir} ---")
    
    if not os.path.exists(data_dir):
        print(f"Error: Directory {data_dir} not found. Please run the generator script.")
        exit()
    ...
    model = DualStreamRadioSqueezeNet(len(CLASS_NAMES)).to(device)
    
    Number_of_parameters = sum(p.numel() for p in model.parameters())
    print(f"Model initialized with {Number_of_parameters:,} parameters (Dual Stream).")
