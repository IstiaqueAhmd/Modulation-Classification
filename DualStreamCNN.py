...
class DualStreamRadioCNN(nn.Module):
    def __init__(self, num_classes):
        super(DualStreamRadioCNN, self).__init__()
        
        # --- Stream 1: Amplitude Branch ---
        # Input: [Batch, 1, 224, 224]
        self.stream_amp = nn.Sequential(
        ...
        # --- Stream 2: Phase Branch ---
        # Input: [Batch, 1, 224, 224]
        self.stream_phase = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=5, stride=2, padding=2),
            nn.BatchNorm2d(32), nn.ReLU(inplace=True), nn.MaxPool2d(2),
        ...
        # --- Fusion & Heads ---
        # 128 features from Amp + 128 features from Phase = 256
    ...
    def forward(self, x):
        ...
        # 2. Process streams independently
...
if __name__ == '__main__':
    # NOTE: Changed to the folder name from your Generator script
    ...
    model = DualStreamRadioCNN(len(CLASS_NAMES)).to(device)
