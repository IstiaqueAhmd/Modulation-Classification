"""
==============================================================
RadioML Generator: Dual-Channel (Amp, Constellation)
==============================================================

Generates 2-channel inputs:
...
def compute_constellation_map(I, Q, size=224):
    ...
    # 2. Fixed Range Histogram [-2.5, 2.5]
...
def normalize_stack_2ch(cwt_ch1, const_ch2):
    """
    Normalize Channel 1 (CWT) but KEEP Channel 2's absolute scaling.
    """
    def norm(arr):
        return (arr - arr.min()) / (arr.max() - arr.min() + 1e-8)
    
    # Channel 1 (Amp) -> Normalize (Relative)
    # Channel 2 (Const) -> USE AS IS (Absolute scaling preserved)
    return np.stack([norm(cwt_ch1), const_ch2], axis=0)

def save_debug_image(cwt_amp, const_map, save_dir, filename):
    """Save visual comparison of the 2 channels."""
    def to_img(arr):
    ...
    # Stack side-by-side: Amp | Constellation
    strip = np.hstack([img_a, img_c])
    cv2.imwrite(os.path.join(save_dir, f"{filename}.png"), strip)
...
def generate_dual_scalograms(snr_list):
    ...
    for snr in snr_list:
        ...
        for mod_class in CLASSES:
            ...
            for fname in files:
                ...
                # 3. Convert to Amplitude ONLY
                amp_sig = np.sqrt(I**2 + Q**2)
                # Phase calculation removed

                # 4. Compute Features (2 Channels)
                cwt_amp = compute_cwt(amp_sig)
                const_map = compute_constellation_map(I, Q, size=224) 

                # 5. Resize CWT to 224x224 (Constellation map is already 224)
                cwt_amp = cv2.resize(cwt_amp, (224, 224), interpolation=cv2.INTER_LANCZOS4)
                
                # 6. Normalize and Stack
                # Returns [2, 224, 224]
                stacked = normalize_stack_2ch(cwt_amp, const_map)
                ...
                if SAVE_SAMPLES and sample_count < NUM_SAMPLES:
                    save_debug_image(cwt_amp, const_map, sample_dir, save_name)
                    sample_count += 1
                
                count += 1

            print(f"   ✓ Generated {count} dual-channel samples.")

    print("\nAll tasks completed successfully. ✅")

if __name__ == "__main__":
    generate_dual_scalograms(SNR_LEVELS)